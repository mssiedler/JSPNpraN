...
<% 
    ...
    //verifico se é excluir
        if(request.getParameter("codigo") != null){
            Joystick obj = dao.buscarPorChavePrimaria(Integer.parseInt(request.getParameter("codigo")));
            if(obj != null){
                dao.excluir(obj);
            }
        }
            ...
            <table class="table table-bordered table-hover">
                <thead>
                    <tr>
                        <th>Código</th>
                        <th>Modelo</th>
                        <th>Quantidade de Botões</th>
                        <th>Console</th>
                        <th >Ações</th>
                    </tr>
                </thead>
                <tbody>
                    <%for(Joystick item: lista)
                    {
                    %>
                    <tr>
                        <td><%=item.getId()%></td>
                        <td><%=item.getModelo()%></td>
                        <td><%=item.getQuantidadeBotoes()%></td>
                        <td><%=item.getNumSerie().getNome() %></td>
